...
test('Review Flow', async ({ page }) => {
  ...
  // Use timestamp to guarantee unique Order ID every run
  const orderId = 'OD' + Date.now();
  const trackingLink = 'TL' + Date.now();
  console.log(`Generated Order ID: ${orderId}, Tracking Link: ${trackingLink}`);
  ...
  // Wait for modal to fully close before interacting with review form
  await page.waitForSelector('.Modal_modalOverlay__vNhYn', { state: 'hidden', timeout: 15000 });

  console.log('💡 Writing recommendation...');
  const recommendationBox = page.getByRole('textbox', { name: 'Share Your Recommendation 💡' });
  await recommendationBox.waitFor({ state: 'visible', timeout: 30000 });
  await recommendationBox.click({ force: true });
